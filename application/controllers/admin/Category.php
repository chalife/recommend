...
class Category extends Admin_Controller {
	...
	public function index()
	{
		...
		if(!empty($admin_id)){
			...
			$config['total_rows'] = $this->Category_model->getCount($keyword);
			$config['per_page'] = 20;
			...
			$this->data['category_show_begin'] = $show_begin;
			$this->data['category_show_end'] = $show_end;
			$this->data['category_total_rows'] = $config['total_rows'];
			$this->data['category_list'] = $this->Category_model->getAll($config['per_page'], $offset, $keyword);
			...
			//加载模板
			$this->template->admin_load('admin/category/index', $this->data);
		}else{
	...
	public function view($id)
	{
		...
		//获取数据
		$blog = $this->Category_model->find($id);
		if(empty($blog)){
		...
		if(strripos($form_url,"admin/category/index") === FALSE){
		...
		//加载模板
		$this->template->admin_load('admin/category/view', $this->data);
	}
	...
	public function del($id=0)
	{
		...
		$data['blog'] = $blog;
		$this->load->view('admin/category/modals/del', $data);
	}
	...
	public function edit($id = 0)
	{
		...
		//加载模板
		$this->template->admin_load('admin/category/edit', $this->data);
	}
	...
	public function create()
	{
		...
		if($this->input->method() == "post"){
			...
			// 表单校验
			$this->form_validation->set_rules('category_name', '标签分类', 'required|min_length[2]|max_length[20]');
		...
		//加载模板
		$this->template->admin_load('admin/category/create', $this->data);
	}
...
//                    'updated'	=> date('Y-m-d H:i:s'),
		...
		$data['blog'] = $blog;
		$this->load->view('admin/category/modals/reset_password', $data);
	}
	...
	public function integral()
	{
		...
		if(!empty($admin_id)){
			...
			$base_url = base_url('/admin/category/integral');
			if(!empty($keyword)){
				$base_url .="?keyword=".$keyword;
			}
			$config['base_url'] = $base_url;
			...
			//加载模板
			$this->template->admin_load('admin/category/integral', $this->data);
		}else{
			redirect("/admin");
		}
	...
	public function rides()
	{
		...
		if(!empty($admin_id)){
			...
			$base_url = base_url('/admin/category/rides');
			if(!empty($keyword)){
				$base_url .="?keyword=".$keyword;
			}
			$config['base_url'] = $base_url;
			$config['total_rows'] = $this->Category_model->find_rides_count_by_mobile($keyword);
			$config['per_page'] = 20;
			...
			$this->data['rides_list'] = $this->Category_model->find_rides_by_mobile($config['per_page'], $offset, $keyword);
			...
			//加载模板
			$this->template->admin_load('admin/category/rides', $this->data);
		}else{
			redirect("/admin");
		}
	...
	public function evaluation()
	{
		...
		if(!empty($admin_id)){
			...
			$base_url = base_url('/admin/category/evaluation');
			if(!empty($keyword)){
				$base_url .="?keyword=".$keyword;
			}
			$config['base_url'] = $base_url;
			...
			//加载模板
			$this->template->admin_load('admin/category/evaluation', $this->data);
		}else{
			redirect("/admin");
		}
