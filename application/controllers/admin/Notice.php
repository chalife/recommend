...
class notice extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if (!empty($admin_id)) {
            ...
            $base_url = base_url('/admin/notice/index');
            if (!empty($keyword)) {
                $base_url .= "?keyword=" . $keyword;
            }
            $config['base_url'] = $base_url;
            $config['total_rows'] = $this->notice_model->getCount($keyword);
            $config['per_page'] = 20;
            ...
            $this->data['administrators_list'] = $this->notice_model->getAll($config['per_page'], $offset, $keyword);
            ...
            //加载模板
            $this->template->admin_load('admin/notice/index', $this->data);
        } else {
    ...
    public function edit()
    {
        ...
        if ($this->input->method() == "post") {
            ...
            $data['admin_id'] = $this->checkLogin('A');
            ...
            if ($id) {
                $this->data['data'] = $this->notice_model->find($id);
            }

            $this->template->admin_load('admin/notice/edit', $this->data);
    ...
    public function del()
    {
        ...
        $data['id'] = $id;
        $this->load->view('admin/notice/modals/del', $data);
