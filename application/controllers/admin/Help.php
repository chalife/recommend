...
class Help extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        if (!empty($admin_id)) {
            ...
            $base_url = base_url('/admin/help/index');
            if (!empty($keyword)) {
                $base_url .= "?keyword=" . $keyword;
            }
            $config['base_url'] = $base_url;
            $config['total_rows'] = $this->help_model->getCount($keyword);
            $config['per_page'] = 20;
            ...
            $this->data['administrators_list'] = $this->help_model->getAll($config['per_page'], $offset, $keyword);
            ...
            //加载模板
            $this->template->admin_load('admin/help/index', $this->data);
        } else {
            redirect("/admin/help");
        }
        //加载模板
    ...
    public function edit()
    {
        ...
        if ($this->input->method() == "post") {
            ...
            if ($id) {
                $this->data['data'] = $this->help_model->find($id);
            }

            $this->template->admin_load('admin/help/edit', $this->data);
    ...
    public function del()
    {
        ...
        $data['id'] = $id;
        $this->load->view('admin/help/modals/del', $data);
